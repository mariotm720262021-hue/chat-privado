...
export default function App() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      await loginWithOAuth("google", currentDynamicUrl);
    } catch (err: any) {
      console.error(err);
      setAuthError(
        err.message || 
        `Error al conectar con Google. Asegúrate de agregar '${window.location.origin}' en Redirect URLs en el panel de Supabase.`
      );
    } finally {
      setAuthLoading(false);
    }
          ...
          >
            ...
            <div className="mt-6 pt-5 border-t border-slate-800/80">
              ...
              <button
                ...
                disabled={authLoading}
                className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-slate-800 hover:bg-slate-700/90 border border-slate-700/80 rounded-xl text-sm font-semibold text-slate-100 transition-all hover:border-slate-600 disabled:opacity-50 shadow-md active:scale-[0.99]"
              >
                <GoogleIcon />
